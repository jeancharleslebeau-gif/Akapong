
#include "common.h"
#include "lib/expander.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "soc/soc_caps.h"
#include "esp_log.h"
#include "driver/i2s_std.h"
#include <string.h> // <cstring> en C++

#include "lib/lcd.h"
#include "lib/audio.h"
#include "TAS2505_rehs.h"

// #define USE_MCLK_ON_GPIO42    // map MCLK on GPIO 42 @ 11 Mhz : same sound

static void delay(uint32_t u32_ms)
{
    vTaskDelay(u32_ms / portTICK_PERIOD_MS);
}

static i2s_chan_handle_t tx_chan; // I2S tx channel handler

int16_t cos44100[] = {
#include "cos44100.h"
};

extern void read_audio_file(uint8_t *ptr, uint32_t u32_length);
int16_t i16_fifo[GB_AUDIO_BUFFER_SAMPLE_COUNT * GB_AUDIO_BUFFER_FIFO_COUNT];
// uint8_t u8_fifo[1];
uint32_t u32_idx_read = 0;
uint32_t u32_idx_write = 0;
uint32_t inline fifo_size() { return count_of(i16_fifo); }
uint32_t inline fifo_used() { return u32_idx_write - u32_idx_read; }
uint32_t inline fifo_free() { return fifo_size() - fifo_used(); }

// #define BLOCK_WAV_READ 512  // 512 bytes at a time
uint16_t u16_buffer_miss = 0;    // count missing data during playback
uint16_t u16_buffer_overvlw = 0; // count surnum data during playback

void wav_pool_update()
{
    while (fifo_free() >= GB_AUDIO_BUFFER_SAMPLE_COUNT)
    {
        read_audio_file((uint8_t *)&i16_fifo[u32_idx_write % fifo_size()], 2 * GB_AUDIO_BUFFER_SAMPLE_COUNT);
        u32_idx_write += GB_AUDIO_BUFFER_SAMPLE_COUNT;
    }
}

void audio_push_buffer(const int16_t *pi16_audio_buffer)
{
    if (fifo_free() >= GB_AUDIO_BUFFER_SAMPLE_COUNT)
    {
        memcpy(&i16_fifo[u32_idx_write % fifo_size()], pi16_audio_buffer, 2 * GB_AUDIO_BUFFER_SAMPLE_COUNT);
        /*
                int16_t* pi16_psrc = pi16_audio_buffer;
                int16_t* pi16_dest = &i16_fifo[u32_idx_write%fifo_size()];
                for ( uint16_t u16_idx = 0 ; u16_idx < GB_AUDIO_BUFFER_SAMPLE_COUNT; u16_idx++ )
                {
        //            *pi16_dest++ = *pi16_psrc++;
                    int16_t i16_data = *pi16_psrc++;
                    *pi16_dest++ = i16_data;
                }
        */
        u32_idx_write += GB_AUDIO_BUFFER_SAMPLE_COUNT;
    }
    else
        u16_buffer_overvlw++;
}

int i32_i2S_loop = 0;
uint32_t *pBuff;
uint32_t pSize;
uint16_t u16_read_ptr = 0;
float f32_pitch = 256 / 4;
uint16_t u16_pitch = 0;
IRAM_ATTR bool i2s_cb(i2s_chan_handle_t handle, i2s_event_data_t *event, void *user_ctx)
{
    i32_i2S_loop++;
    pSize = event->size;
    pBuff = event->dma_buf;
    int16_t *pSamples = event->dma_buf;
    uint16_t pWordLength = event->size / 2;

    if (0)
    {
        while (pWordLength--)
        {
            *pSamples++ = cos44100[u16_read_ptr >> 8];
            u16_read_ptr += u16_pitch;
        }
    }
    else
    {
        if (fifo_used() >= pWordLength)
        {
            memcpy(pSamples, &i16_fifo[u32_idx_read % fifo_size()], 2 * GB_AUDIO_BUFFER_SAMPLE_COUNT);
            /*int16_t* pSrc = &i16_fifo[u32_idx_read%fifo_size()];
            while ( pWordLength-- )
            {
//                uint16_t u16_src = *pSrc++;
//                u16_src = (u16_src<<8) | (u16_src>>8);
//                *pSamples++ =  u16_src ; // marche po
                    *pSamples++ = *pSrc++;
            }
            */
            u32_idx_read += pWordLength; // event->size;
        }
        else
        {
            u16_buffer_miss++;
            while (pWordLength--)
                *pSamples++ = 0;
        }
    }

    return true;
}

i2s_event_callbacks_t i2s_event_callbacks = {
    .on_recv = 0,
    .on_recv_q_ovf = 0,
    .on_sent = i2s_cb,
    .on_send_q_ovf = 0};
int init_i2S()
{
    /* Setp 1: Determine the I2S channel configuration and allocate two channels one by one
     * The default configuration can be generated by the helper macro,
     * it only requires the I2S controller id and I2S role
     * The tx and rx channels here are registered on different I2S controller,
     * Except ESP32 and ESP32-S2, others allow to register two separate tx & rx channels on a same controller */
    //    i2s_chan_config_t tx_chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NUM_AUTO, I2S_ROLE_MASTER);
    i2s_chan_config_t tx_chan_cfg = {
        .id = I2S_NUM_AUTO,
        .role = I2S_ROLE_MASTER,
        .dma_desc_num = 2,
        .dma_frame_num = GB_AUDIO_BUFFER_SAMPLE_COUNT, // 256 x 16b samples
        .auto_clear_after_cb = false,
        .auto_clear_before_cb = false,
        .intr_priority = 3,
    };

    ESP_ERROR_CHECK(i2s_new_channel(&tx_chan_cfg, &tx_chan, NULL));

    /* Step 2: Setting the configurations of standard mode and initialize each channels one by one
     * The slot configuration and clock configuration can be generated by the macros
     * These two helper macros is defined in 'i2s_std.h' which can only be used in STD mode.
     * They can help to specify the slot and clock configurations for initialization or re-configuring */
    i2s_std_config_t tx_std_cfg = {
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(GB_AUDIO_SAMPLE_RATE),

        .slot_cfg = I2S_STD_MSB_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO),
        .gpio_cfg = {
#ifdef USE_MCLK_ON_GPIO42
            .mclk = GPIO_NUM_42, // I2S_GPIO_UNUSED,    // some codecs may require mclk signal, this example doesn't need it
#else
            .mclk = I2S_GPIO_UNUSED, // some codecs may require mclk signal, this example doesn't need it
#endif
            .bclk = I2S_PIN_BCLK,
            .ws = I2S_PIN_WS,
            .dout = I2S_PIN_DOUT,
            .din = I2S_GPIO_UNUSED,
            .invert_flags = {
                .mclk_inv = false,
                .bclk_inv = false,
                .ws_inv = false,
            },
        },
    };
    tx_std_cfg.slot_cfg.bit_shift = true;
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(tx_chan, &tx_std_cfg));

    //    ESP_ERROR_CHECK(i2s_channel_preload_data(tx_chan, cos44100, 256, &w_bytes));
    i2s_channel_register_event_callback(tx_chan, &i2s_event_callbacks, (void *)0);
    ESP_ERROR_CHECK(i2s_channel_enable(tx_chan));

    return 0;
}

// const uint16_t u16_samples[] = { 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080, 0x0100, 0x0200, 0x0400, 0x0800, 0x1000, 0x2000, 0x4000, 0x8000 };
// const uint16_t u16_samples[] = { 0x0001, 0x0003, 0x0007, 0x000F, 0x001F, 0x003F, 0x007F, 0x00FF, 0x01FF, 0x03FF, 0x07FF, 0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF, 0xFFFF };

// uint8_t u8_volume = 0; // max
int audio_init()
{
    printf("audio_test()\n");
    wav_pool_update();
    expander_audio_amplifier_reset(0);
    delay(100);
    expander_audio_amplifier_reset(1);
    delay(100);

    u16_pitch = f32_pitch;
    init_i2S();

    // Page switch to Page 0
    audio_amp_write(AUDIO_AMP_REG_PAGE, 0); // W 30 00 00
    // Assert Software reset (P0, R1, D0=1)
    audio_amp_write(AUDIO_AMP_SOFT_RESET, 1); // W 30 01 01

    // Page Switch to Page 1
    audio_amp_write(AUDIO_AMP_REG_PAGE, 1); // W 30 00 01
    // LDO output programmed as 1.8V and Level shifters powered up. (P1, R2, D5-D4=00, D3=0)
    audio_amp_write(AUDIO_AMP_P1_LDO_CTRL, 0); // W 30 02 00

    // Page switch to Page 0
    audio_amp_write(AUDIO_AMP_REG_PAGE, 0); // W 30 00 00
    // PLL_clkin = BCLK, codec_clkin = PLL_CLK, MCLK should be 11.2896MHz (P0, R4, D1-D0=03)
#ifdef USE_MCLK_ON_GPIO42
    audio_amp_write(AUDIO_AMP_P0_CLK_SETTING1, 3); // w 30 04 03
#else
    audio_amp_write(AUDIO_AMP_P0_CLK_SETTING1, 4 + 3); // w 30 04 03
#endif

    // Power up PLL, set P=1, R=1, (Page-0, Reg-5)
    //    audio_amp_write( AUDIO_AMP_P0_CLK_SETTING2, 0x91 ); // w 30 05 91
    // Power up PLL, set P=1, R=4, (Page-0, Reg-5)
    //    audio_amp_write( AUDIO_AMP_P0_CLK_SETTING2, 0x94 ); // w 30 05 91

    // Power up PLL, set P=1, R=3, (Page-0, Reg-5)
#ifdef USE_MCLK_ON_GPIO42
    audio_amp_write(AUDIO_AMP_P0_CLK_SETTING2, 0x91); // w 30 05 91
#else
    audio_amp_write(AUDIO_AMP_P0_CLK_SETTING2, 0x93); // w 30 05 91
#endif

    // Set J=4, (Page-0, Reg-6)
    //    audio_amp_write( 6, 0x04 ); // w 30 06 04
    // Set J=20, (Page-0, Reg-6)
    audio_amp_write(6, 20); // w 30 06 04

    // D = 0000, D(13:8) = 0, (Page-0, Reg-7)
    audio_amp_write(7, 0x00); // w 30 07 00
                              // D(7:0) = 0, (Page-0, Reg-8)
    audio_amp_write(8, 0x00); // w 30 08 00
                              // add delay of 15 ms for PLL to lock
    delay(15);
    // DAC NDAC Powered up, NDAC=4 (P0, R11, D7=1, D6-D0=0000100)
    audio_amp_write(0x0B, 0x84); // W 30 0B 84

    // DAC MDAC Powered up, MDAC=2 (P0, R12, D7=1, D6-D0=0000010)
    audio_amp_write(0x0C, 0x82); // W 30 0C 82

    // DAC OSR(9:0)-> DOSR=128 (P0, R12, D1-D0=00)
    audio_amp_write(0x0D, 0x00); // W 30 0D 00
                                 // DAC OSR(9:0)-> DOSR=128 (P0, R13, D7-D0=10000000)
    audio_amp_write(0x0E, 0x80); // W 30 0E 80

    // Codec Interface control Word length = 16bits, BCLK&WCLK inputs, I2S mode. (P0, R27, D7-D6=00, D5-D4=00, D3-D2=00)
    audio_amp_write(AUDIO_AMP_AIS_REG1, 0x00); // W 30 1B 00
                                               // Data slot offset 00 (P0, R28, D7-D0=0000)
    audio_amp_write(0x1C, 0x00); // W 30 1C 00
                                 // Dac Instruction programming PRB #2 for Mono routing. Type interpolation (x8) and 3 programmable Biquads. (P0, R60, D4-D0=0010)
    audio_amp_write(0x3C, 0x02); // W 30 3C 02

    // Page switch to Page 0
    audio_amp_write(AUDIO_AMP_REG_PAGE, 0); // W 30 00 00
                                            // DAC powered up, Soft step 1 per Fs. (P0, R63, D7=1, D5-D4=01, D3-D2=00, D1-D0=00)
    audio_amp_write(0x3F, 0x90); // W 30 3F 90
                                 // DAC digital gain 0dB (P0, R65, D7-D0=00000000)
    audio_amp_write(0x41, 0x00); // W 30 41 00
                                 // DAC volume not muted. (P0, R64, D3=0, D2=1)
    audio_amp_write(0x40, 0x04); // W 30 40 04

    // Page Switch to Page 1
    audio_amp_write(AUDIO_AMP_REG_PAGE, 0x01); // W 30 00 01
                                               // Master Reference Powered on (P1, R1, D4=1)
    audio_amp_write(0x01, 0x10); // W 30 01 10
                                 // Output common mode for DAC set to 0.9V (default) (P1, R10)
    audio_amp_write(0x0A, 0x00); // W 30 0A 00
                                 // Mixer P output is connected to HP Out Mixer (P1, R12, D2=1)
    audio_amp_write(0x0C, 0x04); // w 30 0C 04
                                 // HP Volume, 0dB Gain (P1, R22, D6-D0=0000000)
    audio_amp_write(0x16, 0x00); // W 30 16 00
                                 // No need to enable Mixer M and Mixer P, AINL Voulme, 0dB Gain (P1, R24, D7=1, D6-D0=0000000)
    audio_amp_write(0x18, 0x00); // W 30 18 00
    audio_amp_write(0x19, 0x00); // W 30 18 00
                                 // Power up HP (P1, R9, D5=1)
    audio_amp_write(0x09, 0x20); // w 30 09 20

    // Unmute HP with 0dB gain (P1, R16, D4=1)
    audio_amp_write(0x10, 0x00); // w 30 10 00
                                 //    audio_amp_write( 0x10, 0x06 ); // HP Driver Gain : +6dB no effect
                                 //    audio_amp_write( 0x10, 0x0C ); // HP Driver Gain : +12dB no effect

    // SPK attn. Gain =0dB (P1, R46, D6-D0=000000)
    audio_amp_write(AUDIO_AMP_P1_SPK_VOL, 0); // 32 dB att : set to max

    // SPK driver Gain=6.0dB (P1, R48, D6-D4=001)
    //    audio_amp_write( AUDIO_AMP_P1_SPK_AMP_VOL, 0x10 ); // +6 dB, ok
    audio_amp_write(AUDIO_AMP_P1_SPK_AMP_VOL, 0x20); // + 12 dB, ok sur 1W
                                                     //    audio_amp_write( AUDIO_AMP_P1_SPK_AMP_VOL, 0x30 ); // + 18 dB, ok sur 2W
                                                     //    audio_amp_write( AUDIO_AMP_P1_SPK_AMP_VOL, 0x40 ); // + 24 dB, saturé sur 2W
                                                     //    audio_amp_write( AUDIO_AMP_P1_SPK_AMP_VOL, 0x50 ); // + 32 dB, grosse saturation

    // SPK powered up (P1, R45, D1=1)
    audio_amp_write(AUDIO_AMP_REG_PAGE, 0x01);   // W 30 00 01
    audio_amp_write(AUDIO_AMP_P1_SPK_AMP, 0x02); // W 30 2D 02

    /*for (;;)
    {
        delay(20);
        printf( "Fifo data %ld / %ld\n", fifo_used(), fifo_size() );
        wav_pool_update();
//        printf( "I2S loop %d / %d |vol = %d\n", i32_i2S_loop, i32_i2S_loop_ok, u8_volume );
        //printf( "I2S loop %d ptr=%p, len=%ld\n", i32_i2S_loop, pBuff, pSize );
//        printf( "Fifo data %ld / %ld\n", fifo_used(), fifo_size() );

        uint16_t u16_button = expander_read();
        if ( u16_button & EXPANDER_KEY_UP ) {
            if (u8_volume )
                u8_volume--;
            audio_amp_write( AUDIO_AMP_P1_SPK_VOL, u8_volume );
            lcd_draw_str(0, 0, "EXPANDER_KEY_UP ");
            lcd_printf( "volume %d\n", u8_volume );
            lcd_refresh();
            }
        if ( u16_button & EXPANDER_KEY_DOWN ) {
            if (u8_volume<255 )
                u8_volume++;
            audio_amp_write( AUDIO_AMP_P1_SPK_VOL, u8_volume );
            lcd_draw_str(0, 0, "EXPANDER_KEY_DOWN ");
            lcd_printf( "volume %d\n", u8_volume );
            lcd_refresh();
        }
        if ( u16_button & EXPANDER_KEY_RIGHT ) {
            u8_volume=255;
            audio_amp_write( AUDIO_AMP_P1_SPK_VOL, u8_volume );
            lcd_draw_str(0, 0, "EXPANDER_KEY_RIGHT ");
            lcd_printf( "volume %d\n", u8_volume );
            lcd_refresh();
        }
        if ( u16_button & EXPANDER_KEY_LEFT ) {
            u8_volume=64;
            audio_amp_write( AUDIO_AMP_P1_SPK_VOL, u8_volume );
            lcd_draw_str(0, 0, "EXPANDER_KEY_LEFT ");
            lcd_printf( "volume %d\n", u8_volume );
            lcd_refresh();
        }

        if ( u16_button & EXPANDER_KEY_A ) {
            f32_pitch *= 1.02;
            u16_pitch = f32_pitch;
        }
        if ( u16_button & EXPANDER_KEY_B ) {
            f32_pitch *= 1.0/1.02;
            u16_pitch = f32_pitch;
        }


        if ( u16_button & EXPANDER_KEY_RUN ) {
            printf( "Power off...\n" );
            expander_power_off();
        }

    }
    */
    /*
    for (int i = 0; i < 255 ; i++ )
    {
        printf( "Write reg \n" );
        audio_amp_write( 0, (uint8_t)i );
        delay(1000);
        printf( "Read reg \n" );
        uint8_t reg_data = audio_amp_read( 0 );
        printf( "Audio page reg = %d\n", reg_data );
        delay(1000);
    }
    */
    return 0;
}

uint32_t audio_fifo_buffer_count(void)
{
    return count_of(i16_fifo) / GB_AUDIO_BUFFER_SAMPLE_COUNT;
}

uint32_t audio_fifo_buffer_used(void)
{
    return (u32_idx_write - u32_idx_read) / GB_AUDIO_BUFFER_SAMPLE_COUNT;
}

uint32_t audio_fifo_buffer_free(void)
{
    return audio_fifo_buffer_count() - audio_fifo_buffer_used();
}
