	...
	*/
	...
	void pinMode(gpio_num_t pin, uint8_t u8_level)
	{
		gpio_config_t io_conf = {
			...
			.mode = (u8_level == OUTPUT) ? GPIO_MODE_OUTPUT : GPIO_MODE_INPUT
		};
		gpio_config(&io_conf);
	}
	...
	*/
	...
	void lcd_draw_char(uint16_t x, uint16_t y, char c) {
		for (uint16_t dy = 0; dy < 8; dy++) {
			...
			uint16_t swapped = (current_text_color >> 8) | (current_text_color << 8);
			for (uint16_t dx = 0; dx < 8; dx++) {
				if (u8_line & 1) {
					lcd_putpixel(x+dx, y+dy, swapped);
				}
				u8_line >>= 1;
			}
		} 
	...
	void lcd_draw_char_bg(uint16_t x, uint16_t y, char c, uint16_t bgColor) {
		for (uint16_t dy = 0; dy < 8; dy++) {
			uint8_t u8_line = font8x8_basic[(uint8_t)c][dy];
			uint16_t swapped = (current_text_color >> 8) | (current_text_color << 8);
			uint16_t swappedBg = (bgColor >> 8) | (bgColor << 8);
			for (uint16_t dx = 0; dx < 8; dx++) {
				if (u8_line & 1) {
					lcd_putpixel(x+dx, y+dy, swapped); // texte
				} else {
					lcd_putpixel(x+dx, y+dy, swappedBg);            // fond
				}
				u8_line >>= 1;
			}
		}
